...
public class ArtemisRESTClient extends AbstractArtemisClient implements IMappingLoader {
    ...
    public ArtemisRESTClient(final String username, final String password, final String hostName) {
        ...
        this.deserializingObjectMapper = new ObjectMapper().configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES,
                false);
    ...
    public void downloadExerciseAndSubmission(IExercise exercise, ISubmission submission, File directory,
        ...
        final File projectDirectory = projectFileNamingStrategy.getProjectFileInWorkspace(directory, exercise,
                submission);
        try {
            ...
            this.downloadSubmission(submission,
                    projectFileNamingStrategy.getAssignmentFileInProjectDirectory(projectDirectory));
    ...
    private String getApiRoot() {
        return new StringBuilder(Constants.HTTPS_PREFIX).append(this.getArtemisHostname())
            .append("/api")
            .toString();
    }
    ...
    public IAssessor getAssessor() throws ArtemisClientException {
        ...
        final Response rsp = this.rootApiTarget.path("users")
    ...
    @Override
    public Collection<ICourse> getCourses() throws ArtemisClientException {
        this.checkAuthentication();
        final Response rsp = this.rootApiTarget.path(COURSES_PATHPART)
            .request()
            .header(AUTHORIZATION_NAME, this.idToken.get()
                .getHeaderString())
            .buildGet()
    ...
    @Override
    public Collection<IExam> getExamsForCourse(ICourse course) throws ArtemisClientException {
        final Response examsRsp = this.rootApiTarget.path(COURSES_PATHPART)
    ...
    @Override
    public Collection<IExerciseGroup> getExerciseGroupsForExam(IExam exam, ICourse course)
    ...
    @Override
    public Collection<IExercise> getExercisesForCourse(ICourse course) throws ArtemisClientException {
        this.checkAuthentication();
        final Response exercisesAndParticipationsRsp = this.rootApiTarget.path(COURSES_PATHPART)
            .path(String.valueOf(course.getCourseId()))
    ...
    @Override
    public Collection<ISubmission> getSubmissions(int exerciseID, boolean assessedByTutor)
            throws ArtemisClientException {
        this.checkAuthentication();
    ...
    @Override
    public Collection<ISubmission> getSubmissionsForExercise(IExercise exercise) throws ArtemisClientException {
        this.checkAuthentication();
        final Response rsp = this.rootApiTarget.path(EXERCISES_PATHPART)
    ...
    private boolean isStatusSuccessful(final Response response) {
        return Family.SUCCESSFUL.equals(response.getStatusInfo()
            .getFamily());
    }

    private void login() throws ArtemisClientException {
        final Response authenticationResponse = this.rootApiTarget.path("authenticate")
            .request()
    ...
    @Override
    public void saveAssessment(int participationID, boolean submit, String payload) throws ArtemisClientException {
        this.checkAuthentication();

        final Response rsp = this.rootApiTarget.path("participations")
    ...
    public ILockResult startAssessment(int submissionID) throws ArtemisClientException {
        ...
        final Response rsp = this.rootApiTarget.path(PROGRAMMING_SUBMISSION_PATHPART)
    ...
    @Override
    public Optional<ILockResult> startNextAssessment(int exerciseID, int correctionRound)
            throws ArtemisClientException {
        this.checkAuthentication();
    ...
    private void throwIfStatusUnsuccessful(final Response response) throws ArtemisClientException {
        if (!this.isStatusSuccessful(response)) {
            throw new ArtemisClientException("Communication with \"" + this.getApiRoot() + "\" failed with status \""

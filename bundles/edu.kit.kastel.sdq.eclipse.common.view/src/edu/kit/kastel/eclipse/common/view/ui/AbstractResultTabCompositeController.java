...
abstract class AbstractResultTabCompositeController extends AbstractResultTabComposite {
    ...
    protected void setSuccessAndScore(IExercise currentExercise, boolean successOfAutomaticTests, double points,
            double score, String completionTime) {
        var display = getDisplay();
        String title = currentExercise == null ? I18N().unknownTask() : currentExercise.getTitle();

        this.lblStateOfTests.setForeground(successOfAutomaticTests ? display.getSystemColor(SWT.COLOR_GREEN)
                : display.getSystemColor(SWT.COLOR_RED));
        this.lblStateOfTests.setText(I18N().tests() + " "
                + (successOfAutomaticTests ? I18N().successful() : I18N().unsuccessful()));
    ...
    protected void createTableItemsForFeedback(Table table, String name, Feedback feedback) {
        String roundedCredits = feedback.getCredits() == null ? "0.00"
                : String.format(Locale.ENGLISH, "%.2f", feedback.getCredits());

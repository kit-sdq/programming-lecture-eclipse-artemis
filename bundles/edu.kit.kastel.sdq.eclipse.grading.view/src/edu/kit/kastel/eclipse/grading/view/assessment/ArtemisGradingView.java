...
public class ArtemisGradingView extends ViewPart {
    ...
    private void fillBacklogComboWithData(Combo backlogCombo, Combo filterCombo) {
        ...
        if (idx >= 0) {
            ...
            filter = Arrays.stream(SubmissionFilter.values())
                .filter(f -> f.name()
                    .equals(value))
                .findFirst()
                .get();
        }
        this.viewController.getSubmissionsForBacklog(filter)
    ...
    private void resetCombos() {
        ...
        this.viewController.getCourseShortNames()
            .forEach(courseShortName -> this.courseCombo.add(courseShortName));
    }
}

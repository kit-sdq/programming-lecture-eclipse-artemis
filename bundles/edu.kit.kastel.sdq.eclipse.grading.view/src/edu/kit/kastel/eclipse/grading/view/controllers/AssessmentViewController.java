...
/**
 * This class is the controller for the grading view. It creates the marker for the annotations and
 * holds all controller for the backend calls.
 *
...
public class AssessmentViewController {
    ...
    public AssessmentViewController() {
        this.systemwideController = Activator.getDefault()
            .getSystemwideController();
        this.alertObserver = new ViewAlertObserver();
        this.artemisGUIController = this.systemwideController.getArtemisGUIController();
        this.systemwideController.getAlertObservable()
            .addAlertObserver(this.alertObserver);
        this.artemisGUIController.getAlertObservable()
            .addAlertObserver(this.alertObserver);
    }

    /**
     * This method creates a marker for the annotation and add a new annotation to the backlog
     *
     * @param mistake
     *            (the mistake type of the new annotation)
     * @param customMessage
     *            (for custom mistake type, else null)
     * @param customPenalty
     *            (for custom mistake, else null)
     * @param ratingGroupName
     *            (the name of the rating group of the new annotation)
     */
    public void addAssessmentAnnotaion(IMistakeType mistake, String customMessage, Double customPenalty,
        ...
        try {
            IMarker marker = AssessmentUtilities.getCurrentlyOpenFile()
            ...
            marker.setAttribute(AssessmentUtilities.MARKER_ATTRIBUTE_RATING_GROUP,
                    mistake == null ? ratingGroupName
                            : mistake.getRatingGroup()
                                .getDisplayName());
    ...
    private void createMarkerForAnnotation(IAnnotation annotation) {
        ...
        String customMessage = annotation.getCustomMessage()
            .orElse("");
        Double customPenalty = annotation.getCustomPenalty()
            .orElse(0.0);
        try {
            ...
            if (mistake != null) {
                ...
                marker.setAttribute(AssessmentUtilities.MARKER_ATTRIBUTE_ERROR, mistake.getName());
                marker.setAttribute(AssessmentUtilities.MARKER_ATTRIBUTE_RATING_GROUP, mistake.getRatingGroup()
                    .getDisplayName());
                marker.setAttribute(IMarker.MESSAGE,
                        AssessmentUtilities.createMarkerTooltip(startLine + 1, endLine + 1, mistake.getName(),
                                mistake.getRatingGroup()
                                    .getDisplayName(),
                                mistake.getMessage(), annotation.getClassFilePath()));
            }
        } catch (Exception e) {
    ...
    /**
     * @param courseTitle
     *            (of the selected course in the combo)
    ...
    /**
     * @param examShortName
     *            (of the selected exam in the combo)
    ...
    /**
     * reloads the current assessment and creates the marker for the given annotations
     */
    ...
    /**
     * creates a new assessment controller (if needed) and adds a observer for error handling
     */
    public void setCurrentAssessmentController() {
        ...
        this.assessmentController.getAlertObservable()
            .addAlertObserver(this.alertObserver);
    }

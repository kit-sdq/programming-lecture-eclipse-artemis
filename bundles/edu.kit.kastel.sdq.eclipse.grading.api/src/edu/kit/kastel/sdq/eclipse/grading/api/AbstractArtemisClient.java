...
public abstract class AbstractArtemisClient {
    ...
    /**
     *
     * @param artemisUsername
     *            for login to artemis
     * @param artemisPassword
     *            for login to artemis
     * @param artemisHostname
     *            the hostname, only! (e.g. "test.kit.edu")
     */
     ...
     *
     * @param exerciseID
     * @param assessedByTutor
     *            only return those submissions on which the caller has (started, saved or
    ...
    /**
     * Submit the assessment to Artemis. Must have been started by {@link #startAssessment(int)} or
     * {@link #startNextAssessment(int, int)} before!
     *
     * @param participationID
     *            THOU SHALT NOT PROVIDE THE SUBMISSIONID, HERE! The participationID can be gotten
     *            from the {@link ILockResult}, via {@link #startAssessment(int)} or
     *            {@link #startNextAssessment(int, int)}! * @param submit determine whether the
     *            assessment should be submitted or just saved.
     * @param payload
     *            the payload formatted correctly. TODO ISubmission statt ID TODO Payload
     *            verstecken, irgendein Vorberechnungsergebnis
     * @throws ArtemisClientException
     */
    public abstract void saveAssessment(int participationID, boolean submit, String payload)
            throws ArtemisClientException;
     ...
     *
     * @param submissionID
     * @return the data gotten back, which is needed for submitting the assessment result correctly
    ...
    /**
     * Starts an assessment for any available submission (determined by artemis). Acquires a lock in
     * the process.
     *
     * @param correctionRound
     *            relevant for exams! may be 0 or 1
     * @return
     *         <li>the data gotten back. Needed for submitting correctly.
     *         <li><b>null</b> if there is no submission left to correct
     * @throws ArtemisClientException
     *             if some errors occur while parsing the result or if authentication fails.
     */
    public abstract Optional<ILockResult> startNextAssessment(int exerciseID, int correctionRound)
            throws ArtemisClientException;
}
